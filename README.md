...
These circuits let a holder prove, in zero knowledge and with quantum-resistant cryptography,
that it knows the seed — or an intermediate key — from which an account was derived, without
revealing it. A quantum-weak ECDSA account can be authenticated this way instead, by proving
...
The public API is stable, but this implementation has not undergone an external
